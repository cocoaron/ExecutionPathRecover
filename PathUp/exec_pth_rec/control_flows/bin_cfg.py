...
class BinCFGGenerator:
    ...
    def get_node_reboot(self, program, cfg, name):
        ...
        for item in reboots:
            if name==item:
                try:
                    # Split from the right to handle cases with extra colons
                    reboot_name, reboot_addr = item.rsplit(":", 1)
                    addr = int(reboot_addr, 16)  # Convert address to int
                    node = cfg.get_any_node(addr, anyaddr=True)
                    if node:
                        found_nodes.append(node)
                except ValueError:
                    print(f"Skipping malformed reboot entry: {item}")
        return found_nodes
