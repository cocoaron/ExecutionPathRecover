...
/sbin/cmdigmp start
# Delete the static route for DNS
local ipv4_wantype=\$(/bin/config get wan_proto)
if [ "x\$ipv4_wantype" = "xpptp" ]; then
	IF=\$(cat /etc/ppp/peers/dial-provider | grep ^pptp_iface |cut -f2 -d' ')
	GW=\$(cat /etc/ppp/peers/dial-provider | grep ^pptp_gateway |cut -f2 -d' ')
	DNS=\$(cat /tmp/resolv.conf |grep ^nameserver | cut -f2 -d' ')
	for i in \$DNS; do
		ip route del \$i via \$GW dev \$IF
	done
fi
EOF
}
...
setup_interface_ppp() {
	...
	if [ "$($CONFIG get GUI_Region)" = "Russian" ]; then
		echo "$language" >> /etc/ppp/peers/dial-provider
	fi
	if [ "$($CONFIG get wan_pptp_dns_assign)" != "0" ]; then
	...
	if [ "$($CONFIG get wan_pppoe_wan_assign)" = "1" ]; then
		[ "x$($CONFIG get GUI_Region)" = "xRussian" ] &&  netmask=$($CONFIG get wan_pppoe_eth_mask)
